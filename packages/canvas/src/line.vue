<template>
    <canvas :id="id" :style="styleObject">
    </canvas>
</template>
<script>
export default {
    ...
    mounted() {
      console.log("parent",this.$parent,"el",this.$el)
        ...
        const width = this.width ? this.width : this.$parent.$el.offsetWidth ? this.$parent.$el.offsetWidth :window.innerWidth;
        const height = this.height ? this.height: this.$parent.$el.offsetHeight ?  this.$parent.$el.offsetHeight : window.innerHeight;
        var base_list = [];
        canvas.width = width;
